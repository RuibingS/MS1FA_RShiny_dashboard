...
featureTable.import.fun <- function(file.dir) {
  ...
  if ("row.ID" %in% FT_colnames) { 
    ...
    required_cols <- c("row.ID", "row.m.z", "row.retention.time")
    missing_cols <- setdiff(required_cols, colnames(featureTable))
    
    if (length(missing_cols) > 0) {
      stop(paste("Missing columns in input data:", paste(missing_cols, collapse = ", ")))
    }
    
    colnames(featureTable)[which(colnames(featureTable) == "row.ID")] <- "feature_name"
    colnames(featureTable)[which(colnames(featureTable) == "row.m.z")] <- "mz"
    colnames(featureTable)[which(colnames(featureTable) == "row.retention.time")] <- "rt"
    
    featureTable$feature_name <- sprintf("FT%04d", as.integer(featureTable$feature_name))
    featureTable$mz <- as.numeric(featureTable$mz)
    
    featureTable$rt <- suppressWarnings(as.numeric(featureTable$rt))  
    if (any(is.na(featureTable$rt))) {
      warning("Some retention time values could not be converted to numeric.")
    }
    
    featureTable$rt <- round(featureTable$rt * 60, 3)
...
#FT_test <- featureTable.import.fun(here::here("Data","feature_table", "StM16_XCMS_FT_MS1_New.csv"))


#demoStM16_FT_MZmine_test <- featureTable.import.fun( here::here("Data", "feature_table","MZmine_StM16_FT_export_MS2.csv"))
